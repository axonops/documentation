...
for node in $nodes; do
    ...
    # Check current cache size via SSH
    entries=$(ssh "$node" "nodetool info" 2>/dev/null | grep -A 1 "Row Cache" | grep "entries" | awk '{print $4}')
    echo "  Current entries: $entries"

    # Invalidate via SSH
    echo "  Invalidating..."
    ssh "$node" "nodetool invalidaterowcache"
    ...
    # Check recovery
    hit_rate=$(ssh "$node" "nodetool info" 2>/dev/null | grep -A 2 "Row Cache" | grep "hit rate" | awk '{print $4}')
...
if [ "$confirm" = "yes" ]; then
    for node in $nodes; do
        echo -n "$node: "
        ssh "$node" "nodetool invalidaterowcache" 2>/dev/null && echo "invalidated" || echo "FAILED"
    done
fi
```
