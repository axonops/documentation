---
title: "Restore a whole cluster from a remote backup with different IP addresses"
...
> The commands above assume you are storing the Cassandra data in the default location `/var/lib/cassandra`. Update the paths if your data is stored elsewhere.

As the IP addresses of the replacement Cassandra nodes are different from the old cluster, you will need to update the
