...
description: "Run axon-agent in Docker containers. Container deployment guide."
meta:
  - name: keywords
    content: "axon-agent Docker, container deployment, Docker agent"
---
...
To enable the full functionality of the axon-agent some directories must be accessible to both the Cassandra and
axon-agent processes.
...
| `/var/lib/cassandra` | Optional | For the backups feature to function correctly the Cassandra data directory must be readable by the axon-agent                                                                      |

When running Cassandra under Docker it is possible to run the axon-agent either on the host or in another 
...
This example shows running a single Cassandra node and the axon-agent under Docker Compose using host volumes
