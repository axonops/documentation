...
```bash
nodetool -h 192.168.1.100 setcompactionthreshold my_keyspace my_table 4 32
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node setcompactionthreshold $KEYSPACE $TABLE $MIN_THRESHOLD $MAX_THRESHOLD \
