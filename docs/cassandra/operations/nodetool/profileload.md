...
```bash
nodetool -h 192.168.1.100 profileload my_keyspace my_table 60000
```
...
nodetool -h hot_node profileload myapp main_table 60000

# Compare with other nodes
nodetool -h other_node profileload myapp main_table 60000
```
...
for node in $nodes; do
    ...
    nodetool -h $node profileload $KEYSPACE $TABLE $DURATION > "$OUTPUT_DIR/profile_$node.txt" &
done
...
nodetool -h problem_node profileload my_keyspace my_table 60000
