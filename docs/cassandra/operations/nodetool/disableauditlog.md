...
```bash
nodetool -h 192.168.1.100 disableauditlog
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node disableauditlog 2>/dev/null && echo "disabled" || echo "FAILED"
done
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node getauditlog 2>/dev/null | grep "enabled"
done
```
