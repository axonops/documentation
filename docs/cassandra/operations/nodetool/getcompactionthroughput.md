...
```bash
nodetool -h 192.168.1.100 getcompactionthroughput
```
...
for node in node1 node2 node3; do
    echo -n "$node: "
    nodetool -h $node getcompactionthroughput
done
```
...
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    throughput=$(nodetool -h $node getcompactionthroughput 2>/dev/null | awk '{print $4}')
...
# Audit cluster
for node in node1 node2 node3; do
    echo "$node: $(nodetool -h $node getcompactionthroughput | awk '{print $4}')"
done
...
# Standardize
nodetool -h node2 setcompactionthroughput 64
```
