...
```bash
nodetool -h 192.168.1.100 invalidatekeycache
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node invalidatekeycache 2>/dev/null && echo "invalidated" || echo "FAILED"
done
...
for node in $nodes; do
    ...
    nodetool -h $node invalidatekeycache

    echo "Waiting ${WAIT_TIME}s for cache to warm..."
    sleep $WAIT_TIME

    # Check hit rate before proceeding
    hit_rate=$(nodetool -h $node info 2>/dev/null | grep -A 1 "Key Cache" | grep "hit rate" | awk '{print $4}')
