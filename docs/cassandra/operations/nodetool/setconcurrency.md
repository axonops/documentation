...
```bash
nodetool -h 192.168.1.100 setconcurrency read 48
```
...
for node in $nodes; do
    echo "=== $node ==="
    nodetool -h $node setconcurrency read $READ_CONCURRENCY
    nodetool -h $node setconcurrency write $WRITE_CONCURRENCY
    nodetool -h $node getconcurrency
    echo ""
done
```
