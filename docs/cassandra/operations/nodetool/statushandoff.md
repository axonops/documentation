...
```bash
nodetool -h 192.168.1.100 statushandoff
```
...
for node in $nodes; do
    status=$(nodetool -h $node statushandoff 2>/dev/null)
    echo -n "$node: "
...
# Synchronize by running on all nodes:

for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    nodetool -h $node enablehandoff
done

# Verify consistency
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    echo -n "$node: "
    nodetool -h $node statushandoff
done
```
