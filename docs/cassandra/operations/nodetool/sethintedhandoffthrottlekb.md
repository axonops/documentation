...
```bash
nodetool -h 192.168.1.100 sethintedhandoffthrottlekb 1024
```
...
# Monitor target node
nodetool -h <target_node> tpstats
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node sethintedhandoffthrottlekb $THROTTLE && echo "OK" || echo "FAILED"
done
```
