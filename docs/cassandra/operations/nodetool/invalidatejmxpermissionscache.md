...
```bash
nodetool -h 192.168.1.100 invalidatejmxpermissionscache
```
...
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    ...
    nodetool -h $node invalidatejmxpermissionscache
done
...
echo "Invalidating JMX permissions cache cluster-wide..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node invalidatejmxpermissionscache 2>/dev/null && echo "invalidated" || echo "FAILED"
done
...
echo "Refreshing JMX access caches cluster-wide..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo "Processing $node..."
    nodetool -h $node invalidatejmxpermissionscache 2>/dev/null
