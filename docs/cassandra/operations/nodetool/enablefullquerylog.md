...
```bash
nodetool -h 192.168.1.100 enablefullquerylog --path /var/log/cassandra/fql
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node enablefullquerylog --path $FQL_PATH 2>/dev/null && echo "enabled" || echo "FAILED"
done
...
for node in $nodes; do
    echo "=== $node ==="
    nodetool -h $node getfullquerylog 2>/dev/null | head -5
done
```
