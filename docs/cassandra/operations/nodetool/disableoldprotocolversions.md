...
```bash
nodetool -h 192.168.1.100 disableoldprotocolversions
```
...
echo "Disabling old protocol versions cluster-wide..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node disableoldprotocolversions 2>/dev/null && echo "disabled" || echo "FAILED"
done
...
echo "Checking protocol version status across cluster..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo "=== $node ==="
    nodetool -h $node clientstats 2>/dev/null | head -10
    echo ""
done
```
...
nodetool -h <node_ip> disableoldprotocolversions
