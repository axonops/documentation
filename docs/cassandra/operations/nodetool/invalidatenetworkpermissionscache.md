...
```bash
nodetool -h 192.168.1.100 invalidatenetworkpermissionscache
```
...
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    ...
    nodetool -h $node invalidatenetworkpermissionscache
    nodetool -h $node invalidatecidrpermissionscache
done
...
echo "Invalidating network permissions cache cluster-wide..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node invalidatenetworkpermissionscache 2>/dev/null && echo "invalidated" || echo "FAILED"
done
...
echo "Refreshing all network authorization caches cluster-wide..."

nodes=$(nodetool status | grep "^UN" | awk '{print $2}')

for node in $nodes; do
    echo "Processing $node..."
    nodetool -h $node invalidatenetworkpermissionscache 2>/dev/null
    nodetool -h $node invalidatecidrpermissionscache 2>/dev/null
    nodetool -h $node reloadcidrgroupscache 2>/dev/null
    echo "  Done"
done
...
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    nodetool -h $node reloadcidrgroupscache
done
```
