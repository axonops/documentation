...
```bash
nodetool -h 192.168.1.100 gettimeout read
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node gettimeout $TIMEOUT_TYPE 2>/dev/null || echo "FAILED"
done
```
...
for type in read write range counterwrite; do
    ...
    values=""
    for node in $nodes; do
        value=$(nodetool -h $node gettimeout $type 2>/dev/null | grep -oE "[0-9]+" | head -1)
        echo "  $node: ${value}ms"
        values="$values $value"
    done
