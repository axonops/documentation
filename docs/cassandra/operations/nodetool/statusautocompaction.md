...
```bash
nodetool -h 192.168.1.100 statusautocompaction my_keyspace
```
...
for node in $nodes; do
    echo "=== Node: $node ==="
    disabled=$(nodetool -h $node statusautocompaction 2>/dev/null | grep "running=false")
    if [ -z "$disabled" ]; then
...
for node in $nodes; do
    status=$(nodetool -h $node statusautocompaction $KEYSPACE $TABLE 2>/dev/null | awk '{print $2}')
    echo "$node: $status"
...
for node in $(nodetool status | grep "^UN" | awk '{print $2}'); do
    nodetool -h $node enableautocompaction my_keyspace my_table
done
```
