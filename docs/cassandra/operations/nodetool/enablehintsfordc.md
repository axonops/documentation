...
```bash
nodetool -h 192.168.1.100 enablehintsfordc dc2
```
...
for node in $nodes; do
    echo -n "$node: "
    nodetool -h $node enablehintsfordc $DATACENTER && echo "enabled" || echo "FAILED"
done
```
...
# 2. Re-enable hints for this DC on all nodes
echo ""
