...
```bash
nodetool -h 192.168.1.100 failuredetector
```
...
for node in $nodes; do
    ...
    nodetool -h $node failuredetector 2>/dev/null || echo "Cannot connect to $node"
done
```
...
nodetool -h <node_ip> tpstats

# Check for GC issues
nodetool -h <node_ip> gcstats
```
